...
PROF_NAME="aws-wmiso-inventory:aws-wmiso-inventory-ir"
ROLE_ARN="arn:aws:iam::${TARGETACCTID}:role/${TARGETROLE}"
ROLE_SESSION_NAME=${SESSIONNAME}
CREDS=$(aws sts assume-role --profile ${PROF_NAME} --role-arn ${ROLE_ARN} --duration-seconds ${DURATION} ${EXTERNALID}  --role-session-name ${ROLE_SESSION_NAME})
if [ "$?" -ne "0" ]; then
